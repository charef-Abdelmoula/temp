...
class ImportBillInv04(models.TransientModel):
    _name = 'import.bill.inv04'
    _description = 'Import Bill INV04'

    file = fields.Binary('File')
    import_warning = fields.Text('Import Summary', readonly='1')

    @api.multi
    def do_import_bill_inv04(self):
...
# Customer Number
            ...
            journal_id = journal_pool.search([('code','=','BILL04')])
            if not journal_id:
                raise except_orm('Journal Not Found!', 'No journal found with sort code BILL04')

            ordering_date = row[col_map['orderingdate']].value

            if ordering_date:
                ordering_date = xlrd.xldate_as_tuple(row[col_map['orderingdate']].value, book.datemode)
...
#             invoice_number = row[col_map['InvoiceNumber']].value.strip()
            ...
            if invoice_date:
                invoice_date = xlrd.xldate_as_tuple(row[col_map['invoicedate']].value, book.datemode)
                invoice_date = str(invoice_date[0]) + '-' + str(invoice_date[1]) + '-' + str(invoice_date[2])
#                 invoice_date = parse(invoice_date, dayfirst=True)
                invoice_date = datetime.strftime(datetime.strptime(invoice_date, '%Y-%m-%d'), DEFAULT_SERVER_DATE_FORMAT)
            ...
            if paid_on:
                paid_on = xlrd.xldate_as_tuple(row[col_map['paidon']].value, book.datemode)
                paid_on = str(paid_on[0]) + '-' + str(paid_on[1]) + '-' + str(paid_on[2])
#                 paid_on = parse(paid_on, dayfirst=True)
                paid_on = datetime.strftime(datetime.strptime(paid_on, '%Y-%m-%d'), DEFAULT_SERVER_DATE_FORMAT)
            ...
            if 'customernumber' in col_map:
                ...
                if customernumber and customernumber not in customer_map:
                    customer_id = customer_pool.create({'name': customer_firstname+ ' ' + customer_surname,
                                                                         'ppc_name' : customer_firstname+ ' ' + customer_surname,
        ...
        import_id = self.env['import.bill.inv04'].create({'import_warning': Warning_msg, 'warning': True})

        return {
            'name': 'BILL04 Import',
            'view_mode': 'form',
            'res_id': import_id.id,
            'res_model': 'import.bill.inv04',
            'view_type': 'form',
