...
class ImportInv04(models.TransientModel):
    _name = 'import.inv04'
    _description = 'Import INV04'

    file = fields.Binary('File')
    import_warning = fields.Text('Import Summary', readonly='1')

    @api.multi
    def do_import_inv04(self):
...
# Customer Number
            ...
            journal_id = journal_pool.search([('code','=','S4DE')])
            if not journal_id:
                raise except_orm('Journal Not Found!', 'No journal found with sort code S4DE')
...
#                 'ppc_paidon' : paid_on,
        ...
        import_id = self.env['import.inv04'].create({'import_warning': Warning_msg, 'warning': True})

        return {
            'name': 'INV04 Import',
            'view_mode': 'form',
            'res_id': import_id.id,
            'res_model': 'import.inv04',
            'view_type': 'form',
